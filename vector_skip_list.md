...
The idea is simple. We will use skip list tree. The differnet stuff is that under each node, it is not a single key, but a vector of keys. Each node is represented by the smallest key in the vector. The algorithm will guarentee all keys for one node will be less than the next node's represented key, i.e., the smallest key for the next node is greater than all keys in the previous node. So by this way, the nodes of the linked list are sorted but the keys in the vector of one node are not sorted. 

And all keys are distinct.
...
After the modification of the tree, you can new another Immutable Iterator for integrity.
